...
The real difference lies in B cell/MHC II response. B cell receptors recognize fully folded post-translationally modified proteins. B cells are also APCs so they can phagocytose stuff and present it via MHC II. MHC II presents a wider range of material (including very often glycopeptides) and larger peptides up to 25 amino acids long. CD4+ T cells interact with MHC II on APCs (B cells, dendritic cells, macrophages, etc) to coordinate both CD 8+ and B cell responses. MRNA vaccines attempt to mimic quaternary structure with contiguous amino acids that are not post translationally modified. The targets most likely have to be external protein antigens that are not folded into the center of the surface protein complex. But, many of the best TAAs are post-translationally modified. MRNA vaccines worked for covid spike protein because it was secreted from infected cells. Spike protein is a well-studied protein that didn't really change much from strain to covid strain and it is so huge that finding an external unmodified contiguous chain of amino acids was comparatively easy. The problem of finding a unique external contiguous unmodified sequence is much larger for cancer, but possibly doable. mRNA technology can indeed bind the antigen to a peptide that is likely to get presented on MHCII (chimera proteins) but it seems less likely to present native quaternary structures and doesn’t account for post-translational modifications. This strategy may attain comparable MHCII presentation efficiency if the right adjuvants and chemically optimization/liposomes/nanoparticles are used. Experimental work is needed to optimize the strategy.
...
| **Immune Processing** | Relies on host cells to express and process peptide antigens | Fragments are naturally phagocytosed and processed via lysosomes |
