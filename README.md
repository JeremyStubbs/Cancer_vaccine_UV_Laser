...
It is similar to _in situ_ vaccination after radiation therapy, something that has shown promising results in early studies. The main differences are the location of light exposure (inside vs outside the patient) and the frequency of light (UV vs Ionizing radiation). Compared to in situ vaccination following radiation therapy the possible advantages are: greater control over antigen size, the option to filter unwanted material and the option to chemically optimize the fragments (optimize immunogenicity, optimize stability, and liposome formation).

Vaccines based on mRNA to the best of my knowledge attempt to generate peptides approximately 20 amino acids in length. The idea is that at that length there are 20^20 possible combinations of peptides. Surely, there are unique mutations that are not found anywhere else in the body of that length which you can train the immune system to recognize. One challenge is: in the tumor cell, are these readily available for interaction with the immune system? They would have to be either external protein antigens that are not folded into the center of the surface protein complex Or internal protein antigens that are processed and presented by the tumor cells themselves on MHCI complexes. Often point mutations can lead to misfolding. Immune cells recognize the fully folded protein and that mutation may be part of a quaternary structure (interchain hydrogen bonding) with distance chains. The mRNA would have to code for something that simulates the area with the mutation that contains distant chains folded together. Admittedly, AI can predict stuff way better than humans, but at this point it feels like you are oversimplifying complexity. Particularly when you consider that many proteins are post-translationally modified.

With this strategy you do not need to reduce the fragment size down to 15-20 amino acids, which is the length of peptide that MHC molecules present. After phagocytosis, lysosomal processing will do that and select the best antigen candidates with full post-translational processing. Breaking the cells into bite sized bits is the key and seems fully doable. The natural target for this approach is pathology slices.
